    ...
    if (bits > 0 && bits <= 1024)
{
    ...
    {
        static if (bits >= 16)
            ArbInt!bits result = cast(ArbInt!bits)((hash >> bits) ^ (hash & arbMask(bits)));
        else
            ArbInt!bits result = cast(ArbInt!bits)(((hash >> bits) ^ hash) & arbMask(bits));
    }
    else
